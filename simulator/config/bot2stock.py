...
from agent.BotmasterAgent import BotmasterAgent
...
parser.add_argument('--bots', action='store', type=int, default=1,
                    help='Number of spoofing bots to use.')
...
# Number of spoofing bots to use
num_spoofers = args.bots
...
random_state = lambda: np.random.RandomState(seed=np.random.randint(low=0,high=2**32))
...
print ("Shock variance: ............ {:0.4f}\n".format(sigma_s))
print ("Perform Attack: ............ {}".format(attack))
...
defaultComputationDelay = 1000000  # 10 milliseconds
...
kernel = Kernel("Base Kernel", random_state=random_state())
...
mkt_close = midnight + pd.to_timedelta('09:30:02.5')
...
# Create the exchange.
num_exchanges = 1
agents.extend([ExchangeAgent(j, "Exchange Agent {}".format(j), "ExchangeAgent", mkt_open, mkt_close, [s for s in symbols],
                             log_orders=log_orders, book_freq=book_freq, pipeline_delay=0, computation_delay=0,
                             stream_history=10, random_state=random_state())
...
# Some configs that mix both ZI and HBL agents.

# 11 agents
#zi = [ (1, 0, 250, 1), (1, 0, 500, 1), (1, 0, 1000, 0.8), (1, 0, 1000, 1), (1, 0, 2000, 0.8), (1, 250, 500, 0.8), (1, 250, 500, 1) ]
...
for i,x in enumerate(zi):
    ...
    agents.extend([ZeroIntelligenceAgent(j, "ZI Agent {} {}".format(j, strat_name), "ZeroIntelligenceAgent {}".format(strat_name),
                   random_state=random_state(), log_orders=log_orders, symbol=symbol, starting_cash=starting_cash,
                   sigma_n=sigma_n, r_bar=s['r_bar'], kappa=s['kappa'], sigma_s=s['sigma_s'], q_max=10, sigma_pv=5000000,
                   R_min=x[1], R_max=x[2], eta=x[3], lambda_a=lambda_a)
...
for i,x in enumerate(hbl):
    ...
    agents.extend([HeuristicBeliefLearningAgent(j, "HBL Agent {} {}".format(j, strat_name),
                   "HeuristicBeliefLearningAgent {}".format(strat_name), random_state=random_state(), log_orders=log_orders,
                   symbol=symbol, starting_cash=starting_cash, sigma_n=sigma_n, r_bar=s['r_bar'], kappa=s['kappa'],
                   sigma_s=s['sigma_s'], q_max=10, sigma_pv=5000000, R_min=x[1], R_max=x[2], eta=x[3], lambda_a=lambda_a, L=x[4])
            for j in range(agent_count,agent_count+x[0])])
...
# Botmaster
spoof_time = midnight + pd.to_timedelta('09:30:00')
spoof_shares = 10000

for j in range(1):
    i = agent_count
    botmaster_id = agent_count
    agents.append(BotmasterAgent(i, "Botmaster Agent {}".format(i), "BotmasterAgent", attack_time=spoof_time, lambda_a=lambda_a,
                                     symbol="IBM", starting_cash=starting_cash, log_orders=False, random_state=random_state()))
    agent_types.append("Botmaster Agent {}".format(i))
    agent_count += 1
...
if attack:
    for j in range(num_spoofers):
        i = agent_count
        agents.append(SpoofingAgent(i, "Spoofing Agent {}".format(i), "SpoofingAgent", spoof_time=None, botmaster=botmaster_id,
                                    spoof_shares=spoof_shares, symbol="IBM", starting_cash=starting_cash, log_orders=False,
                                    random_state=random_state(), lambda_a=lambda_a))

        agent_types.append("Spoofing Agent {}".format(i))
        agent_count += 1
