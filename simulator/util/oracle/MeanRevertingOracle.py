...
class MeanRevertingOracle:
  ...
  def __init__(self, mkt_open, mkt_close, symbols):
    ...
    # Oracle automatically adjusts its granularity based on the simulation duration
    self.freq = None
    self.freq_choices = [('L', '1ms'), ('S', '1s')]
  ...
  def generate_fundamental_value_series(self, symbol, r_bar, kappa, sigma_s):
    ...
    for freq in self.freq_choices:
        try:
            date_range = pd.date_range(self.mkt_open, self.mkt_close, closed='left', freq=freq[0])
            self.freq = freq
            break
        except MemoryError:
            continue
  ...
  def observePrice(self, symbol, currentTime, sigma_n = 1000, random_state = None):
    ...
    if currentTime >= self.mkt_close:
      r_t = self.r[symbol].loc[self.mkt_close - pd.Timedelta(self.freq[1])]
    else:
      r_t = self.r[symbol].loc[currentTime.round(self.freq[1])]
